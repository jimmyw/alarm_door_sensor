...
#define _40_CGC_HISYS_OSC                                                      \
  (0x40U) /* X1, X2 as crystal/ceramic resonator connection */
#define _80_CGC_HISYS_PORT1 (0x80U) /* X1, X2 as I/O port */
#define _C0_CGC_HISYS_EXT                                                      \
  (0xC0U) /* X1 as I/O port, X2 as external clock input */
...
#define _00_CGC_ILLEGAL_ACCESS_OFF                                             \
  (0x00U) /* disables illegal memory access detection */
...
#define _04_CGC_PORT_GUARD_ON                                                  \
  (0x04U) /* valid, it is impossible to write PORT register, but possible for  \
             read */
/* Interrupt register guard (GINT) */
#define _00_CGC_INT_GUARD_OFF                                                  \
  (0x00U) /* invalid, it is possible to write interrupt register */
#define _02_CGC_INT_GUARD_ON                                                   \
  (0x02U) /* valid, impossible to write interrupt register, but possible for   \
             read */
/* CSC register guard (GCSC) */
#define _00_CGC_CSC_GUARD_OFF                                                  \
  (0x00U) /* invalid, it is possible to write CSC register */
#define _01_CGC_CSC_GUARD_ON                                                   \
  (0x01U) /* valid, it is impossible to write CSC register, but possible for   \
             read */

/***********************************************************************************************************************
Macro definitions
***********************************************************************************************************************/
#define CGC_FIHWAITTIME                                                        \
  (60U) /* change the waiting time according to the system */

#include "r_cg_macrodriver.h"
/***********************************************************************************************************************
Typedef definitions
***********************************************************************************************************************/
typedef enum { HIOCLK, SYSX1CLK, SYSEXTCLK } clock_mode_t;
